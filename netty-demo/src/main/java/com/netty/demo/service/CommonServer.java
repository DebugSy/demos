...
import com.netty.demo.util.TimeDecoder;
 ...
 *
 * 运行main方法,然后在命令行执行telnet localhost 8888
 */
...
//							channel.pipeline().addLast(new DiscardServerHandler());
							channel.pipeline().addLast(new TimeDecoder(),new TimeServerHandler());//注册handler
						}
					})
					/**
					 * 对于ChannelOption.SO_BACKLOG的解释：
					 * 服务器端TCP内核维护有两个队列，我们称之为A、B队列。客户端向服务器端connect时，会发送带有SYN标志的包（第一次握手），
					 * 服务器端接收到客户端发送的SYN时，向客户端发送SYN ACK确认（第二次握手），此时TCP内核模块把客户端连接加入到A队列中，
					 * 然后服务器接收到客户端发送的ACK时（第三次握手），TCP内核模块把客户端连接从A队列移动到B队列，连接完成，
					 * 应用程序的accept会返回。也就是说accept从B队列中取出完成了三次握手的连接。
